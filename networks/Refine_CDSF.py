...
class CDSFW(nn.Module):
    ...
    def __init__(self, num_layers, equi_h, equi_w):
        ...
        self.fusion = Fusion2(self.num_ch_dec[4], 96, self.equi_h//32)
    ...
    def forward(self, input_equi_image, depth_patch_map, roll_idx, flip):
        ...
        # depth_patch_map = torch.unsqueeze(depth_patch_map, dim=1)
        # depth_init = F.interpolate(depth_patch_map, size=(self.equi_h//4, self.equi_w//4)).type(torch.float32)
        # for i in range(input_equi_image.shape[0]):
        #     depth_init[i] = torch.roll(depth_init[i], int(roll_idx[i]), dims=2)
        #     if flip[i] == 1:
        #         depth_init[i] = torch.flip(depth_init[i], [2])
        # depth_init = torch.unsqueeze(depth_init, dim=-1)
