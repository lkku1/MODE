...
def pers2equi(pers_img, fov, nrows, patch_size, erp_size, layer_name, roll_idx, flip):
    # bs = pers_img.shape[0]
    ...
    img_erp = F.grid_sample(pers_img, new_grid, mode='bilinear', padding_mode='border', align_corners=True)
        ...
        new_x_patch = (new_x + 1) * 0.5 * width
        new_y_patch = (new_y + 1) * 0.5 * height
        ...
        mask *= cos_c_mask  # cos_c > 90 and less than patch length
    ...
    output_a = output_a.permute(0, 1, 3, 4, 2)  # bs, 1, h/4, w/4, n_patch
    output_b = output_b.permute(0, 1, 3, 4, 2)
    output_c = output_c.permute(0, 1, 3, 4, 2)
    output_d = output_d.permute(0, 1, 3, 4, 2)

    w_list = w_list.permute(1, 2, 0, 3) #h/4, w/4, n, 4
    w_list = w_list.flatten(3)  #h/4, w/4, n*4
    ...
    #          output_c * w_list[..., 2] + output_d * w_list[..., 3]  #bs, 1, h, w, n_patch
    # output = output.sum(-1).type(torch.float32)


    w_list = w_list.unsqueeze(0)

    for i in range(len(output_a)):
        output_a[i] = torch.roll(output_a[i], int(roll_idx[i]), dims=2)
        output_b[i] = torch.roll(output_b[i], int(roll_idx[i]), dims=2)
        output_c[i] = torch.roll(output_c[i], int(roll_idx[i]), dims=2)
        output_d[i] = torch.roll(output_d[i], int(roll_idx[i]), dims=2)
        w_list_roll = torch.roll(w_list, int(roll_idx[i]), dims=2)
        if flip[i] == 1:
            output_a[i] = torch.flip(output_a[i], [2])
            output_b[i] = torch.flip(output_b[i], [2])
            output_c[i] = torch.flip(output_c[i], [2])
            output_d[i] = torch.flip(output_d[i], [2])
            w_list_flip = torch.flip(w_list_roll, [2])
            output_a[i] = output_a[i] * w_list_flip[..., 0]
            output_b[i] = output_b[i] * w_list_flip[..., 1]
            output_c[i] = output_c[i] * w_list_flip[..., 2]
            output_d[i] = output_d[i] * w_list_flip[..., 3]
        else:
            output_a[i] = output_a[i] * w_list_roll[..., 0]
            output_b[i] = output_b[i] * w_list_roll[..., 1]
            output_c[i] = output_c[i] * w_list_roll[..., 2]
            output_d[i] = output_d[i] * w_list_roll[..., 3]   # bs, 1, h/4, w/4, n_patch

    output = (output_a+output_b+output_c+output_d).type(torch.float32)


    # output = output_a.max(-1)[0].type(torch.float32)
    # overlap = torch.where(torch.sum(mask, dim=0)>1, 1, 0)
    ...
    # output = output.sum(-1)
    # output_a = output_a * w_list[..., 0]
    return output #, output_a, output_b, output_c, output_d #, w_list #, img_erp
