...
from .layers import Conv3x3, ConvBnReLU
from .slicelayer import Resnet, wrap_lr_pad
from collections import OrderedDict
import torch.nn.functional as F
from.consist import CDSF, Fusion, Double_dec, FastUpconv
...
class CDF(nn.Module):
    ...
    def __init__(self, num_layers, equi_h, equi_w):
        ...
        self.equi_dec_convs["upconv_5"] = FastUpconv(equi_c5, self.num_ch_dec[4])

        self.equi_dec_convs["deconv_4"] = ConvBnReLU(self.num_ch_dec[4] + equi_c4, self.num_ch_dec[4])
        self.equi_dec_convs["upconv_4"] = FastUpconv(self.num_ch_dec[4], self.num_ch_dec[3])

        self.equi_dec_convs["deconv_3"] = ConvBnReLU(self.num_ch_dec[3] + equi_c3, self.num_ch_dec[3])
        self.equi_dec_convs["upconv_3"] = FastUpconv(self.num_ch_dec[3], self.num_ch_dec[2])

        self.equi_dec_convs["deconv_2"] = Double_dec(self.num_ch_dec[2], equi_c2) #, self.equi_h//4
        self.equi_dec_convs["upconv_2"] = FastUpconv(self.num_ch_dec[2], self.num_ch_dec[1])

        self.equi_dec_convs["deconv_1"] = ConvBnReLU(self.num_ch_dec[1] + equi_c1, self.num_ch_dec[1])
        self.equi_dec_convs["upconv_1"] = FastUpconv(self.num_ch_dec[1], self.num_ch_dec[0])
        ...
        wrap_lr_pad(self.equi_encoder)
        wrap_lr_pad(self.equi_decoder)
        wrap_lr_pad(self.fusion_net)
        wrap_lr_pad(self.fusion)
    ...
    def forward(self, input_equi_image, depth_patch_map, roll_idx, flip):
        ...
        cds_featue, cds = self.fusion_net(equi_enc_feat2, equi_enc_feat4, depth_init)

        equi_enc_feat5 = self.fusion(equi_enc_feat5, cds_featue)
        outputs = {}
        equi_x = self.equi_dec_convs["upconv_5"](equi_enc_feat5)
        equi_x = self.equi_dec_convs["deconv_4"](torch.cat([equi_enc_feat4, equi_x], dim=1))

        equi_x = self.equi_dec_convs["upconv_4"](equi_x)
        equi_x = self.equi_dec_convs["deconv_3"](torch.cat([equi_enc_feat3, equi_x], dim=1))

        equi_x = self.equi_dec_convs["upconv_3"](equi_x)
        equi_x, seg = self.equi_dec_convs["deconv_2"](equi_enc_feat2, equi_x)
        ...
        outputs['coarse_scene_structure'] = cds
        outputs['skyseg'] = seg
